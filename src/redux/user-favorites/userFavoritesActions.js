...
export const fetchFavoriteMovies = (id, order, currentPage) => {
  return (dispatch) => {
    ...
    axios
      .get(
        `https://api.themoviedb.org/3/account/{account_id}/favorite/movies?api_key=${API_KEY}&language=en-US&session_id=${id}&language=en-US&sort_by=created_at.${order}&page=${currentPage}`
      )
      .then((res) => {
